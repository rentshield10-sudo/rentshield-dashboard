...
//
// This route merges two sources for a complete renewals picture:
...
export async function GET() {
  ...
  try {
    ...
    // Collect leaseIDs that already have a formal renewal record
    const renewedLeaseIds = new Set<string>(
      formalRenewals.map((r) => String((r.lease as Record<string, unknown>)?.leaseID ?? ""))
    );

    // Normalise formal renewals → unified shape
    const renewalRows = formalRenewals.map((r) => {
    ...
    const combined = [...renewalRows, ...expiringRows];

    return NextResponse.json({ ok: true, renewals: combined, total: combined.length });
  } catch (error) {
    return NextResponse.json(
